...
function errorHandler(error: any, _: AuthenticatedRequest, res: Response, __: NextFunction) {
   console.log(error.name);
   switch (error.name) {
      case "MongoError":
         if (error.code === 31254) return res.status(500).json({ status: "error", error: "Internal server serror" });
         else return res.status(400).json({ status: "error", error: "User already exists" });
      case "MulterError":
         return error.code === "LIMIT_FILE_SIZE"
            ...
            : res.status(500).json({ status: "error", error: "Error uploading file" });
      case "JsonWebTokenError":
         return res.status(401).json({ status: "error", error: "You need to be logged in" });
      ...
      case "ProjectInvitationReboundError":
         return res.status(400).json({ status: "error", error: "User is already in the project" });
      case "ValidationError":
         return res.status(400).json({ status: "error", error: "Validation Error" });
      case "IrrevertibleActionError":
         return res.status(400).json({ status: "error", error: "The action is irrevertible" });
      default:
         return res.status(500).json({ status: "error", error: "Internal server error" });
   }
}
